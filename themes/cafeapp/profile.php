...
<div class="app_invoice app_widget">
    ...
    <form class="app_form" action="<?= url("/app/cadastrar"); ?>" method="post">
        ...
        <label>
            ...
            <select name="level">
                <option value="1" <?= ($user->level == 1 ? 'selected' : '') ?>>&ofcir; admin</option>
                <option value="2" <?= ($user->level == 2 ? 'selected' : '') ?>>&ofcir; User</option>
            </select>
        </label>
        <div class="label_group">
            <label>
                <span class="field icon-user">Nome:</span>
                <input class="radius" type="text" name="first_name" value="<?= $user->first_name ?>"
                       placeholder="Primeiro nome" required/>
            </label>

            <label>
                <span class="field icon-user-plus">Sobrenome:</span>
                <input class="radius" type="text" name="last_name" value="<?= $user->last_name ?>"
        ...
        <label>
            ...
            <input class="radius" type="email" name="email" value="<?= $user->email ?>"
                   placeholder="Seu e-mail de acesso" required/>
        </label>
        ...
        <div class="al-center">
            <div>
                <span data-userremove="<?= url("/app/remove-user/{$user->id}") ?>"
